...
public sealed class WindowsPredictionOverlayService : IPredictionOverlayService, IAsyncDisposable
{
    internal const int OverlayMessageBase = 0x8001;
    internal const int OverlayMessageShow = OverlayMessageBase;
    internal const int OverlayMessageUpdate = OverlayMessageBase + 1;
    internal const int OverlayMessageHide = OverlayMessageBase + 2;
    internal const int OverlayMessageShutdown = OverlayMessageBase + 3;

    internal static readonly int WindowExtendedStyles = PredictionOverlayWindowStyles.ExtendedStyles;

    private readonly ILogger<WindowsPredictionOverlayService> _logger;
    ...
    private volatile PredictionOverlayStatus _status = new(PredictionOverlayVisibility.Hidden, false);

    public WindowsPredictionOverlayService(ILogger<WindowsPredictionOverlayService> logger)
    {
        _logger = logger;
    }

    public PredictionOverlayStatus Status => _status;
    ...
    private void EnsureStarted()
    {
        lock (_lifecycleSync)
        {
            ...
            {
                IsBackground = true,
                Name = "SmartInput.PredictionOverlay",
            };
    ...
    private void RunUiThread()
    {
        ...
        try
        {
            ...
            _status = new PredictionOverlayStatus(PredictionOverlayVisibility.Hidden, true);
            _threadReady.TrySetResult();
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Failed to initialize prediction overlay window.");
        ...
        _status = new PredictionOverlayStatus(PredictionOverlayVisibility.Hidden, false);
    }
    ...
    private void DrainWorkQueue(OverlayRenderState renderState)
    {
        while (_workItems.TryDequeue(out var workItem))
        {
            try
            {
                switch (workItem.Kind)
                {
                    case OverlayWorkKind.Show:
                        ...
                        _status = new PredictionOverlayStatus(PredictionOverlayVisibility.Visible, true);
                        break;
                    case OverlayWorkKind.Update:
                        renderState.Update(workItem.Content!, workItem.Placement!);
                        _status = new PredictionOverlayStatus(PredictionOverlayVisibility.Visible, true);
                        break;
            ...
            {
                _logger.LogError(ex, "Prediction overlay work item failed.");
    ...
    private sealed class OverlayWorkItem
    {
        ...
        public PredictionOverlayContent? Content { get; init; }
    ...
    private sealed class OverlayRenderState
    {
        ...
        public OverlayRenderState(nint windowHandle)
        {
            ...
            _font = new Font("Segoe UI", 14f, FontStyle.Italic, GraphicsUnit.Point);
        }

        public void Show(PredictionOverlayContent content, PredictionOverlayPlacement placement)
        {
            Render(content, placement);
        }

        public void Update(PredictionOverlayContent content, PredictionOverlayPlacement placement)
        {
            Render(content, placement);
        }
        ...
        private void Render(PredictionOverlayContent content, PredictionOverlayPlacement placement)
        {
            lock (_renderSync)
            {
                ...
                var width = Math.Max(1, (int)Math.Ceiling(textSize.Width) + 4);
                var height = Math.Max(placement.CaretHeight, (int)Math.Ceiling(textSize.Height) + 2);
                ...
                using var brush = new SolidBrush(Color.FromArgb(120, 100, 100, 100));
                graphics.DrawString(
                    content.SuggestionText,
                    _font,
                    brush,
    ...
    private static class OverlayWindowFactory
    {
        ...
        internal static nint EnsureWindowCreated()
        {
            ...
            if (windowHandle == 0)
            {
                throw new InvalidOperationException("Failed to create prediction overlay window.");
            }

            return windowHandle;
        }

        private const string OverlayWindowClassName = "SmartInputPredictionOverlay";
        ...
        private static void EnsureClassRegistered()
        {
            lock (ClassSync)
            {
                ...
                {
                    throw new InvalidOperationException("Failed to register prediction overlay window class.");
                }
